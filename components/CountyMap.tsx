...
import { countyByFips, type County } from '@/data/counties';
import { getAllCountyThreats, levelColor, type ThreatLevel } from '@/lib/data-loader';
...
const DEFAULT_FILL = '#1a2535';

function hoverColor(level: ThreatLevel): string {
  return {
    low:      '#27b865',
    watch:    '#ada200',
    moderate: '#c46600',
...
export default function CountyMap({ onCountyClick, selectedFips }: Props) {
  ...
  // Load threat data once — it's synchronously available (imported JSON)
  const allThreats = getAllCountyThreats();
      ...
      try {
        ...
        g.selectAll<SVGPathElement, GeoJSON.Feature>('path')
          ...
          .attr('fill', d => {
            ...
            const threat = allThreats[fips];
            return threat ? levelColor(threat.threatLevel) : DEFAULT_FILL;
          })
          .attr('stroke', '#0a1825')
          ...
          .on('mouseenter', function (event: MouseEvent) {
            ...
            d3.select(el)
              .raise()
              .attr('fill', threat ? hoverColor(threat.threatLevel) : '#253547')
          ...
          .on('mouseleave', function (event: MouseEvent) {
            ...
            const threat = allThreats[fips];
            d3.select(el)
              .attr('fill', threat ? levelColor(threat.threatLevel) : DEFAULT_FILL)
              .attr('stroke', '#0a1825')
              .attr('stroke-width', '0.8');
          })
    ...
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      {!loading && (
        <div style={{
          ...
          {[
            { label: 'Clear',    fill: '#27e66e' },
            { label: 'Watch',    fill: '#ffde00' },
            { label: 'Moderate', fill: '#ff8c00' },
            { label: 'Severe',   fill: '#ff4141' },
          ].map(({ label, fill }) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '9px', height: '9px', borderRadius: '50%', background: fill, flexShrink: 0 }} />
              <span style={{ fontSize: '12px', color: 'rgba(226,238,248,0.6)' }}>{label}</span>
            </div>
          ))}
        </div>
      )}
